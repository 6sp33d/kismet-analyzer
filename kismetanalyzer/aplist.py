...
def export_kml(filename, title, devices):
   ...
   """
    ...
    doc = Document(ns=ns, name=title)
    k.append(doc)

    # create placemark for the access point, and add it to the KML document
    for dev in devices:
        icon_style = IconStyle(ns=ns, color=get_networkcolor(dev.encryption))
        style = Style(ns=ns, styles=[icon_style])
        desc = get_description(dev)
        # Convert string coordinates to float for geometry.Point
        try:
            lat = float(dev.location.lat)
            lon = float(dev.location.lon)
            alt = float(dev.location.alt)
            point_geometry = geometry.Point(lat, lon, alt)
            p = Placemark(ns=ns, name=dev.ssid, description=desc, styles=[style], geometry=point_geometry)
